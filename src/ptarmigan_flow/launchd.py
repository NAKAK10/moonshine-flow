...
from ptarmigan_flow.app_bundle import resolve_launch_agent_app_command

LAUNCH_AGENT_LABEL = "com.ptarmiganflow.daemon"
LAUNCHD_LLM_ENABLED_ENV = "PFLOW_LLM_ENABLED"
RESTART_PERMISSION_SUPPRESSION_TTL_SECONDS = 30


def launch_agent_path() -> Path:
    """Path to user LaunchAgent plist."""
    return Path("~/Library/LaunchAgents/com.ptarmiganflow.daemon.plist").expanduser()


def launch_agent_log_paths() -> tuple[Path, Path]:
    """Expected daemon log file locations."""
    log_dir = Path("~/Library/Logs/ptarmigan-flow").expanduser()
...
def launch_agent_restart_suppression_path() -> Path:
    ...
    return (
        Path("~/Library/Application Support/ptarmigan-flow").expanduser()
...
def _resolve_daemon_command() -> list[str]:
    ...
    for candidate in ("pflow", "ptarmigan-flow"):
        ...
        if resolved:
            return [resolved]
    return [sys.executable, "-m", "ptarmigan_flow.cli"]
