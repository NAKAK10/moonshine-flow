...
class RuntimeCandidate:
    ...
    def cli_command(self, cli_args: Sequence[str]) -> list[str]:
        return [str(self.python_path), "-m", "ptarmigan_flow.cli", *cli_args]
...
_PROBE_SCRIPT = textwrap.dedent(
    ...
    try:
        import ptarmigan_flow.cli  # noqa: F401
...
class RuntimeBuilder:
    ...
    def _validate_project_layout(self) -> None:
        ...
        raise RuntimeRepairError(
            ...
            "Try: brew reinstall ptarmigan-flow"
        )
...
class RuntimeManager:
    ...
    def launch(self, cli_args: Sequence[str]) -> int:
        ...
        env_updates[ENV_BOOTSTRAP_SCRIPT] = str(
            (self._project_dir / "src" / "ptarmigan_flow" / "homebrew_bootstrap.py").resolve(
                strict=False
            )
        )
    ...
    def _run_cli_in_current_process(
        ...
        try:
            ...
            sys.argv = ["ptarmigan-flow", *cli_args]
    ...
    def _emit_recovery_notice(
        ...
        print(
            "ptarmigan-flow runtime is unavailable "
        ...
        try:
            process = subprocess.run(
                [
                    str(venv_python),
                    "-c",
                    "from importlib.metadata import version; print(version('ptarmigan-flow'))",
                ],
                check=False,
                capture_output=True,
                text=True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if _is_version_query(cli_args):
        print(f"ptarmigan-flow {_resolve_fast_version(project_dir)}")
        return 0
    ...
    try:
        ...
        print(f"ptarmigan-flow runtime recovery failed: {exc}", file=sys.stderr)
        print("Try: brew reinstall ptarmigan-flow", file=sys.stderr)
        print(
